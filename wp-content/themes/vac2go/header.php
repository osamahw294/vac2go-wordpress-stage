...
<head>
	...
	<!-- Google Consent Mode v2 default + Cookie Notice bridge (must run before any Google tag) -->
	<script>
	(function(){
		...
		// Returning visitor who already accepted: grant immediately, before tags init.
		if (/(?:^|;\s*)cookie_notice_accepted=true(?:;|$)/.test(document.cookie)) {
			grantConsent();
		}

		// New visitor: grant when they click Accept (Cookie Notice legacy markup).
		document.addEventListener('click', function(e){
			var t = e.target;
			if (t && (t.id === 'cn-accept-cookie' || (t.closest && t.closest('#cn-accept-cookie')))) {
				grantConsent();
			}
		}, true);

		// Backstop: Cookie Notice custom event.
		document.addEventListener('setCookieNotice', function(){
			if (/(?:^|;\s*)cookie_notice_accepted=true(?:;|$)/.test(document.cookie)) grantConsent();
		});
	})();
	</script>
<?php
...
?>
	...
	<script async src="https://tag.pearldiver.io/ldc.js?pid=dcbe22363d8179062f802fcdca32e681&aid=16de03f2"></script>
	
	<style>
	     .reserve-menu-icon {
    background-color: #ee2738 !important;
    background: #ee2738 !important;
  }
  
  .reserve-menu-icon > a.mega-menu-link {
  color: #fff !important;
...
.reserve-menu-icon > a.mega-menu-link::before {
  content: "" !important;
  display: inline-block !important;
  width: 18px !important;
  height: 18px !important;
  background-image: url("https://vac2go.com/wp-content/uploads/2026/06/nav-button-icon.png") !important;
...
.reserve-menu-icon > a.mega-menu-link,
.reserve-menu-icon > a.mega-menu-link:hover,
.reserve-menu-icon > a.mega-menu-link:focus,
.reserve-menu-icon > a.mega-menu-link:active {
  background-color: #ee2738 !important;
  background: #ee2738 !important;
  color: #fff !important;
