...
class BG_PROCESS {
    ...
    function set_style() {
        ...
        $background_size = $this->process_values($this->settings['_background_image_size']);
        $background_position = $this->process_values($this->settings['_background_image_position']);
        ...
        if ($this->settings['_use_gradient'] === 'on' || $this->settings['_background_image'] !== '') {
            ET_Builder_Element::set_style($this->options['render_slug'], array(
                ...
                'declaration' => sprintf( 'background-image: %1$s %2$s;
                    ...
                    $background_size,
                    $background_position,
                    $this->process_values($this->settings['_background_image_repeat'])
                )
            ));
        }
    ...
    private function set_hover_styles() {
        $background_size = $this->process_values($this->settings_hover['_background_image_size']);
        $background_position = $this->process_values($this->settings_hover['_background_image_position']);
        ...
        if ($this->settings_hover['_use_gradient'] === 'on' || $this->settings_hover['_background_image'] !== '') {
            ET_Builder_Element::set_style($this->options['render_slug'], array(
                ...
                'declaration' => sprintf( 'background-image: %1$s %2$s;
                    ...
                    $background_size,
                    $background_position,
                    $this->process_values($this->settings_hover['_background_image_repeat'])
                ),
            ));
        }
    }
    ...
    private function background($type = 'default') {
        if ( $this->settings['_above_image'] === 'on' ) {
            return sprintf('%2$s%3$s %1$s',
                $this->background_image($type),
                $this->background_gradient($type),
                $this->seperator
            );
        } else {
     ...
     * Background image
     */
    private function background_image($type) {
     ...
     * 
     */
    private function background_gradient($type) {
        $settings = $type === 'default' ? $this->settings : $this->settings_hover;
        ...
        if ($settings['_use_gradient'] === 'on' ) {
            if ( $settings['_gradient_type'] !== 'radial') {
                ...
                return sprintf('radial-gradient( circle at %3$s, %1$s %4$s, %2$s %5$s )', 
                    $settings['_color_gradient_1'],
                    $settings['_color_gradient_2'],
                    $this->process_values($settings['_radial_direction']),
                    $settings['_start_position'],
                    $settings['_end_position']
                );
            }
     ...
     * @return Array $fields
     */
    public function process_transiton($options) {
